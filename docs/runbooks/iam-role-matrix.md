...
| **GitHub Actions (CI/CD)** | OIDC: tylko token z GitHub Actions dla **konkretnego repo** (i opcjonalnie branch, np. `main`). Principal: `sts.amazonaws.com` + Condition na `token.actions.githubusercontent.com`, claim `sub` (repo, branch). | Least privilege pod Terraform: S3 (bucket stanu: get/put/delete obiektów, w tym plik `.tflock` przy use_lockfile), operacje na zasobach zarządzanych przez Terraform w dev (ECR, ECS, IAM, VPC itd.). Brak dostępu do zasobów poza zakresem pipeline’u. | Workflowy GitHub Actions: `terraform plan` na PR, `terraform apply` na merge do main (lub manual dispatch). Żadnych long-lived keys w secrets. |
